...
#         }
#         if len(lengths) != 1:
#             raise ValueError('All loss-profile arrays must have equal lengths')
...
# def get_loss_profiles(directory: pathlib.Path) -> list[LossProfile]:
#     # file_strings = sorted(
#     #     [str(s) for s in directory.iterdir()],
#     #     key=_natural_keys
#     # )
...
#             if path.is_file() and path.suffix.casefold() == '.csv'
#         ),
#         key=lambda path: _natural_keys(text=path.name),
#     )
...
#         if match is None:
#             raise ValueError(
#                 f'Could not determine maximum elevation from filename: {file.name}'
#             )
...
#                 raise ValueError(f'No headers found in {file}')
#             elif len(headers) != 5:
#                 raise ValueError(f'Expected 5 columns in {file}, got {len(headers)}')
